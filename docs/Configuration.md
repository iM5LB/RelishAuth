...
**Hybrid Method**:
- Password login/register first
- Then required Discord linking before leaving limbo
- Login method chooser is disabled (both factors always required)

[Learn more about authentication methods →](AuthMethods.md)

### Login Method Chooser

When an account has **both** a password and Discord linked:
...
Players type `password` / `discord` (or `1` / `2`) in limbo chat. Works with `method: password` or `discord`; ignored for `hybrid`.
...
- `clear-password-on-discord-unlink`: After Discord unlink, clear the password so it cannot be reused
- `set-password-tip-on-join`: After joining the backend with no password, show a chat tip
- `enforce-discord-account-match`: After unlink, only allow re-linking the same Discord account
...
**Security warning**: Enabling this can allow impersonation of premium accounts unless you add extra authentication checks. See [Security → Premium Account Security](Security.md#premium-account-security).
...
Auth for Bedrock happens in limbo via **titles + chat** (Floodgate/Cumulus forms are not used inside LimboAPI).
...
authentication:
  password:
    hashing: "argon2"  # Options: argon2, bcrypt2y
    argon2:
...
- Higher Argon2 values = more secure, but slower
- Lower values = faster, but less secure
- Default values provide good balance

### Discord → LuckPerms Group Sync (Optional)
...
Requires LuckPerms on the Velocity proxy and a connected Discord bot. Use `/ra syncgroups` to sync the current player.
