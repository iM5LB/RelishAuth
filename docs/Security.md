...
Prevent Discord account switching and reuse of old passwords:
...
**When enabled**:
- Players can only re-link the same Discord account after unlink
- Discord unlink clears the password so it cannot be reused
- Players without a password get a chat tip on backend join
...
5. **Use hybrid auth when you need both factors**:
   ```yaml
   authentication:
     method: "hybrid"
   ```

6. **Strict password requirements**:
   ```yaml
   password:
     min-length: 10
     require-uppercase: true
...
/unlink player:Suspicious
/block username:Griefer from:1.2.3.4
```
