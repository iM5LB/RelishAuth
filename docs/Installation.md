...
- **MySQL/MariaDB/PostgreSQL**: For multi-server database support
- **Floodgate** (+ Geyser): For Bedrock Edition player support
- **LuckPerms** (Velocity): For Discord → group sync
...
2. **Wait for plugins to load**. You should see in console:
   ```
   [INFO] [RelishAuth] Loading RelishAuth v1.1.1
...
3. **Test authentication**:
   - For password: type your password in limbo chat (or `/ra password ...`)
   - For Discord: type your Discord username when prompted
   - If both password and Discord are linked: type `password` or `discord` to choose
