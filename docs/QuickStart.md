...
**For Hybrid (password + Discord required)**:
```yaml
authentication:
  method: "hybrid"
  premium-auto-login: true
...
### For Password Method:
- First-time players: type a new password in limbo chat (confirm when asked)
- Returning players: type your password when prompted
- Or use `/ra password <password> [confirm]`
...
### For Login Chooser (password + Discord already linked):
- Type `password` or `discord` (or `1` / `2`) in limbo chat
...
- [Configure limbo UI](Configuration.md#limbo-world-customization)
