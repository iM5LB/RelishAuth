...
1. **Check authentication method**:
   ```yaml
   authentication:
     method: "password"  # password | discord | hybrid
     login-chooser:
       enabled: true
   ```
...
### Bedrock Forms / Chooser Not Showing

**Symptoms**: Expecting Floodgate forms, or login chooser buttons do nothing

**Explanation**: Cumulus/Floodgate forms cannot be delivered inside LimboAPI. RelishAuth uses limbo titles + chat only.

**Solutions**:
1. Type `password` or `discord` (or `1` / `2`) in limbo chat when the chooser is shown
2. Type the password / Discord username when prompted
3. Do not rely on clickable chat buttons — LimboAPI often drops them

### Discord Interactions Fail (`10062 Unknown interaction`)

**Cause**: Multiple RelishAuth instances using the same bot token, or slow replies

**Solutions**:
1. Run only **one** RelishAuth process per Discord bot token
2. Restart the proxy after fixing duplicates
3. Update to the latest RelishAuth (interactions are acknowledged immediately)

### Unlink Fails / Old Password Still Works

**Solutions**:
1. Ensure `authentication.clear-password-on-discord-unlink: true`
2. For Bedrock, confirm Floodgate is installed and the player username includes the `.` prefix
3. After unlink, set a new password with `/ra password` if needed
