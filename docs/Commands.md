...
### `/ra syncgroups [player]`

Queue a Discord role → LuckPerms group sync for yourself or another online player.

**Usage**:
- Yourself: `/ra syncgroups`
- Another player (admin): `/ra syncgroups PlayerName`

**Permission**: `relishauth.player` (self), `relishauth.admin` (others)

**Requirements**: `group-sync.enabled: true` and LuckPerms on the proxy
...
- If `clear-password-on-discord-unlink: true`, clears the account password so it cannot be reused
...
Commands available through the Discord bot.

### `/password`

Manage your account password (opens a modal).

**Usage**: `/password`

**Permission**: Linked users
...
**Options**: configured presets (typically `0`, `1m`, `5m`, `15m`, `30m`, `1h`)
...
**Usage**:
- View: `/notify`
- Change: `/notify option:on` or `/notify option:off`
...
Unlink Discord (self if no player; admin when targeting others).

**Usage**: `/unlink` or `/unlink player:Notch`

**Permission**: Linked users (self) / Admin role (others)

**Note**: May clear the account password when `clear-password-on-discord-unlink` is enabled.
...
relishauth.command.syncgroups  # /ra syncgroups
